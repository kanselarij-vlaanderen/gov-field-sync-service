...
async function syncFieldsForAllDecisionmakingFlows () {
  // Delete all fields on every decisionmakingFlow
  const deleteString = `
PREFIX besluitvorming: <https://data.vlaanderen.be/ns/besluitvorming#>
DELETE {
    GRAPH ${sparqlEscapeUri(GRAPH)} {
        ?decisionmakingFlow ${sparqlEscapeUri(DECISIONMAKING_FLOW_FIELD_PREDICATE)} ?anyField .
    }
}
WHERE {
    GRAPH ${sparqlEscapeUri(GRAPH)} {
        ?decisionmakingFlow a besluitvorming:Besluitvormingsaangelegenheid ;
            ${sparqlEscapeUri(DECISIONMAKING_FLOW_FIELD_PREDICATE)} ?anyField .
    }
}
  `;

  // Insert all fields from subcases on every decisionmakingFlow
  const insertString = `
PREFIX dossier: <https://data.vlaanderen.be/ns/dossier#>
PREFIX besluitvorming: <https://data.vlaanderen.be/ns/besluitvorming#>
INSERT {
    GRAPH ${sparqlEscapeUri(GRAPH)} {
        ?decisionmakingFlow ${sparqlEscapeUri(DECISIONMAKING_FLOW_FIELD_PREDICATE)} ?field .
    }
}
WHERE {
    GRAPH ${sparqlEscapeUri(GRAPH)} {
...
export {
  ...
  syncFieldsForAllDecisionmakingFlows,
}
